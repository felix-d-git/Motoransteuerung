...
constexpr float  MAX_SPEED      = 2000.0f;//7000.0f;
constexpr float  BESCHLEUNIGUNG   = 1000.0f;
constexpr int    MIN_SPEED      = 100;
constexpr int    BAUD_RATE      = 115200;
constexpr double CM_PER_STEP    = 0.0010055618; // cm pro Schritt; // cm pro Schritt8858; //28697; //0.0000114789;
constexpr long   Overshoot_STEPS = 8700*4;
constexpr int    NUM_MOTORS     = 1;


// PINS
constexpr int STEP_PINS[4]   = {18};
constexpr int DIR_PINS[4]    = {19};
constexpr int SLP_PINS[4]    = { 5};
constexpr int S_UNTEN_PINS[4] = {17};
constexpr int S_OBEN_PINS[4] = {16};
...
void sendeStatus() {
    if (!statusReady) {
        ...
        dtostrf((double)pos * CM_PER_STEP, 1, 2, statusBuf);
        statusReady = true;
    } else {
        // Loop 2: Senden
        Serial.printf("%d%s\n", statusMotor + 1, statusBuf);
        statusMotor = (statusMotor + 1) % NUM_MOTORS;
        statusReady = false;
    }
}
